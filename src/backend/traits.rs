...
// ---------------------------------------------------------------------------
// AsmRelocation — architecture-generic relocation from assembler output
// ---------------------------------------------------------------------------

/// A generic relocation entry produced by the built-in assembler.
///
/// This type bridges the architecture-specific assembler relocation types
/// (e.g., `x86_64::RelocationEntry`) and the linker's `InputRelocation`
/// type.  The `reloc_type` field carries the ELF relocation type value
/// (e.g., `R_X86_64_PC32 = 2`), which is architecture-specific but
/// represented as a plain `u32` for cross-architecture uniformity.
#[derive(Clone, Debug)]
pub struct AsmRelocation {
    /// Byte offset within the function's code where the relocation applies.
    pub offset: usize,
    /// Name of the referenced symbol.
    pub symbol: String,
    /// Architecture-specific ELF relocation type value.
    pub reloc_type: u32,
    /// Signed addend for the relocation computation.
    pub addend: i64,
}

/// Output of the full assembly process including both code and relocations.
#[derive(Clone, Debug)]
pub struct AssemblyOutput {
    /// Encoded machine code bytes.
    pub code: Vec<u8>,
    /// Relocations that require linker resolution.
    pub relocations: Vec<AsmRelocation>,
}
...
pub trait ArchCodegen {
    ...
    /// Assembles a machine function and returns both code and relocations.
    ///
    /// The default implementation delegates to [`emit_assembly`] and returns
    /// an empty relocation list.  Architecture backends that produce
    /// relocations (e.g., for external symbol calls) should override this
    /// to preserve the relocation metadata generated by their assembler.
    fn emit_assembly_with_relocations(&self, mf: &MachineFunction) -> AssemblyOutput {
        AssemblyOutput {
            code: self.emit_assembly(mf),
            relocations: Vec::new(),
        }
    }
