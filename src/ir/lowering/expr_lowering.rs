...
use crate::common::diagnostics::{DiagnosticEngine, Diagnostic, Severity, Span};
#[allow(unused_imports)]
...
use crate::frontend::parser::ast::{
    self as ast_mod, AlignofOperand, BinaryOperator, BlockItem, Expression,
    GenericAssociation, Initializer, InitializerItem, SizeofOperand, Statement,
    TypeName, TypeSpecifier, UnaryOperator, DerivedDeclarator,
    IntegerSuffix, FloatSuffix, StringPrefix, CharPrefix,
};
#[allow(unused_imports)]
use crate::frontend::sema::constant_eval::{
    self as const_eval_mod, ConstValue, evaluate_constant_expression, evaluate_integer_constant,
    is_constant_expression,
};
#[allow(unused_imports)]
use crate::frontend::sema::type_checker::{
    self as type_checker_mod, TypedExpression, is_assignment_compatible,
    insert_implicit_conversion,
};
#[allow(unused_imports)]
...
#[allow(unused_imports)]
use super::{
    LoweringContext, LoweringError, ModuleLoweringContext, c_type_to_ir_type,
...
fn lower_expr_inner(
    ...
    match expr {
        // ---- Literals ----
        Expression::IntegerLiteral { value, suffix, span } => {
            lower_integer_literal(ctx, *value, *suffix, *span)
        }
        Expression::FloatLiteral { value, suffix, span } => {
            lower_float_literal(ctx, *value, *suffix, *span)
        }
        Expression::StringLiteral { value, prefix, span } => {
            lower_string_literal(ctx, value, *prefix, *span)
        }
        Expression::CharLiteral { value, prefix, span } => {
            lower_char_literal(ctx, *value, *prefix, *span)
        }

        // ---- Variable reference ----
        Expression::Identifier { name, span } => {
            lower_variable_ref_rvalue(ctx, *name, *span)
        }
        ...
        Expression::ArrowAccess { pointer, member, span } => {
            lower_member_access_rvalue(ctx, pointer, *member, true, *span)
        }

        // ---- Cast ----
        Expression::Cast { type_name, operand, span } => {
            lower_cast(ctx, type_name, operand, *span)
        }

        // ---- Sizeof / Alignof ----
        Expression::Sizeof { operand, span } => {
            lower_sizeof(ctx, operand, *span)
        }
        Expression::Alignof { operand, span } => {
            lower_alignof(ctx, operand, *span)
        }

        // ---- Compound literal ----
        Expression::CompoundLiteral { type_name, initializer, span } => {
            lower_compound_literal(ctx, type_name, initializer, *span)
        }

        // ---- Comma expression ----
        Expression::Comma { expressions, span } => {
            lower_comma(ctx, expressions, *span)
        }

        // ---- _Generic selection ----
        Expression::Generic { controlling, associations, span } => {
        ...
        // ---- Label address (&&label) ----
        Expression::LabelAddress { label, span } => {
            lower_label_address(ctx, *label, *span)
        }

        // ---- Address-of ----
        Expression::AddressOf { operand, span } => {
            lower_address_of(ctx, operand, *span)
        }

        // ---- Dereference ----
        Expression::Dereference { operand, span } => {
            lower_dereference_rvalue(ctx, operand, *span)
        }
...
fn lower_lvalue_inner(
    ...
    match expr {
        Expression::Identifier { name, span } => {
            lower_variable_ref_lvalue(ctx, *name, *span)
        }
        ...
        Expression::UnaryOp { op: UnaryOperator::Deref, operand, .. } => {
            lower_expression(ctx, operand)
        }
        Expression::StatementExpression { body, span } => {
            lower_statement_expression(ctx, body, *span)
...
fn integer_suffix_to_ir_type(suffix: IntegerSuffix, value: u128, target: &Target) -> IrType {
    match suffix {
        ...
        IntegerSuffix::U => {
            if value <= u32::MAX as u128 { IrType::I32 } else { IrType::I64 }
        }
...
fn size_t_ir_type(target: &Target) -> IrType {
    if target.pointer_width() == 8 { IrType::I64 } else { IrType::I32 }
}
...
fn ir_type_align(ty: &IrType, target: &Target) -> usize {
    match ty {
        ...
        IrType::I64 | IrType::F64 | IrType::Ptr => {
            if target.pointer_width() == 8 { 8 } else { 4 }
        }
...
fn to_i1(ctx: &mut LoweringContext<'_>, val: ValueId) -> ValueId {
    ...
    match &ty {
        ...
        IrType::Ptr => {
            ...
            ctx.builder.build_icmp(ctx.function, ICmpPredicate::Ne, int_val, zero)
        }
        t if is_ir_float(t) => {
            let zero = ctx.builder.build_const_float(ctx.function, ty.clone(), 0.0);
            ctx.builder.build_fcmp(ctx.function, FCmpPredicate::ONe, val, zero)
        }
        t if is_ir_integer(t) => {
            let zero = ctx.builder.build_const_int(ctx.function, ty.clone(), 0);
            ctx.builder.build_icmp(ctx.function, ICmpPredicate::Ne, val, zero)
        }
        _ => {
...
fn lower_string_literal(
    ...
    Ok(ctx.builder.build_global_ref(ctx.function, &name, IrType::Ptr))
}
...
fn lower_variable_ref_rvalue(
    ...
    if let Some(info) = ctx.module_ctx.global_symbols.get(&name) {
        ...
        if let IrType::Function { .. } = &ir_ty {
            return Ok(ctx.builder.build_global_ref(ctx.function, &name_str, IrType::Ptr));
        }
        let gref = ctx.builder.build_global_ref(ctx.function, &name_str, IrType::Ptr);
        return Ok(ctx.builder.build_load(ctx.function, gref, ir_ty));
    }

    ctx.diagnostics().error(span, "use of undeclared identifier");
...
fn lower_binary_op(
    ...
    match op {
        ...
        BinaryOperator::AddAssign => return lower_compound_assign(ctx, BinOp::Add, BinOp::FAdd, left, right, span),
        BinaryOperator::SubAssign => return lower_compound_assign(ctx, BinOp::Sub, BinOp::FSub, left, right, span),
        BinaryOperator::MulAssign => return lower_compound_assign(ctx, BinOp::Mul, BinOp::FMul, left, right, span),
        BinaryOperator::DivAssign => return lower_compound_assign_signaware(ctx, BinOp::SDiv, BinOp::UDiv, BinOp::FDiv, left, right, span),
        BinaryOperator::ModAssign => return lower_compound_assign_signaware(ctx, BinOp::SRem, BinOp::URem, BinOp::SRem, left, right, span),
        BinaryOperator::BitAndAssign => return lower_compound_assign(ctx, BinOp::And, BinOp::And, left, right, span),
        BinaryOperator::BitOrAssign  => return lower_compound_assign(ctx, BinOp::Or, BinOp::Or, left, right, span),
        BinaryOperator::BitXorAssign => return lower_compound_assign(ctx, BinOp::Xor, BinOp::Xor, left, right, span),
        BinaryOperator::ShlAssign => return lower_compound_assign(ctx, BinOp::Shl, BinOp::Shl, left, right, span),
        BinaryOperator::ShrAssign => return lower_compound_assign_signaware(ctx, BinOp::AShr, BinOp::LShr, BinOp::AShr, left, right, span),
    ...
    match op {
        ...
        BinaryOperator::Le | BinaryOperator::Ge => {
            lower_comparison(ctx, op, lhs_val, rhs_val, &lhs_ty, &rhs_ty)
        }
...
fn lower_arithmetic_binop(
    ...
    if matches!(lhs_ty, IrType::Ptr) && is_ir_integer(rhs_ty) && op == BinaryOperator::Add {
        ...
        return Ok(ctx.builder.build_gep(ctx.function, lhs, vec![rhs], IrType::I8, true));
    }
    if is_ir_integer(lhs_ty) && matches!(rhs_ty, IrType::Ptr) && op == BinaryOperator::Add {
        return Ok(ctx.builder.build_gep(ctx.function, rhs, vec![lhs], IrType::I8, true));
    }
    if matches!(lhs_ty, IrType::Ptr) && is_ir_integer(rhs_ty) && op == BinaryOperator::Sub {
        ...
        let neg_rhs = ctx.builder.build_binop(
            ctx.function,
            BinOp::Sub,
            zero_const,
            rhs,
        ...
        return Ok(ctx.builder.build_gep(ctx.function, lhs, vec![neg_rhs], IrType::I8, true));
    }
    ...
    if matches!(lhs_ty, IrType::Ptr) && matches!(rhs_ty, IrType::Ptr) && op == BinaryOperator::Sub {
        ...
        return Ok(ctx.builder.build_binop(ctx.function, BinOp::Sub, lhs_int, rhs_int, int_ty));
    }
    ...
    let ir_op = match op {
        BinaryOperator::Add    => if is_float { BinOp::FAdd } else { BinOp::Add },
        BinaryOperator::Sub    => if is_float { BinOp::FSub } else { BinOp::Sub },
        BinaryOperator::Mul    => if is_float { BinOp::FMul } else { BinOp::Mul },
        BinaryOperator::Div    => if is_float { BinOp::FDiv } else { BinOp::SDiv },
    ...
    Ok(ctx.builder.build_binop(ctx.function, ir_op, lhs_c, rhs_c, result_ty))
}
...
fn lower_comparison(
    ...
    if is_float {
        ...
        let rhs_int = if matches!(rhs_ty, IrType::Ptr) {
            ctx.builder.build_ptr_to_int(ctx.function, rhs, int_ty.clone())
        } else {
...
fn lower_logical_and(
    ...
    ctx.builder.build_cond_branch(ctx.function, lhs_bool, rhs_bb, merge_bb);
    ...
    ctx.builder.add_phi_incoming(ctx.function, phi, false_val, start_bb);
    ctx.builder.add_phi_incoming(ctx.function, phi, rhs_bool, rhs_exit_bb);
...
fn lower_logical_or(
    ...
    ctx.builder.build_cond_branch(ctx.function, lhs_bool, merge_bb, rhs_bb);
    ...
    let phi = ctx.builder.build_phi(ctx.function, IrType::I1);
    ctx.builder.add_phi_incoming(ctx.function, phi, true_val, start_bb);
    ctx.builder.add_phi_incoming(ctx.function, phi, rhs_bool, rhs_exit_bb);
...
fn lower_unary_op(
    ...
    match op {
        ...
        UnaryOperator::Neg => {
            ...
            if is_ir_float(&ty) {
                let zero = ctx.builder.build_const_float(ctx.function, ty.clone(), 0.0);
                Ok(ctx.builder.build_binop(ctx.function, BinOp::FSub, zero, val, ty))
            } else {
                let zero = ctx.builder.build_const_int(ctx.function, ty.clone(), 0);
                Ok(ctx.builder.build_binop(ctx.function, BinOp::Sub, zero, val, ty))
            }
        }
        UnaryOperator::BitNot => {
            ...
            Ok(ctx.builder.build_binop(ctx.function, BinOp::Xor, val, all_ones, ty))
        }
        UnaryOperator::LogNot => {
            let val = lower_expression(ctx, operand)?;
            let bool_val = to_i1(ctx, val);
            let zero = ctx.builder.build_const_int(ctx.function, IrType::I1, 0);
            let inv = ctx.builder.build_icmp(ctx.function, ICmpPredicate::Eq, bool_val, zero);
...
fn lower_pre_inc_dec(
    ...
    if matches!(&ty, IrType::Ptr) {
        ...
        let c = ctx.builder.build_const_int(ctx.function, IrType::I64, if is_inc { 1 } else { -1 });
        let new_ptr = ctx.builder.build_gep(ctx.function, old_val, vec![c], IrType::I8, true);
...
fn resolve_type_name_ir(
    ...
    if specs.len() == 1 {
        match &specs[0] {
            ...
            TypeSpecifier::Long     => {
                return Ok(if target.long_size() == 8 { IrType::I64 } else { IrType::I32 });
            }
    ...
    let has_long_count = specs.iter().filter(|s| matches!(s, TypeSpecifier::Long)).count();
    ...
    if has_void { return Ok(IrType::Void); }
    if has_bool { return Ok(IrType::I1); }
    if has_long_count >= 2 { return Ok(IrType::I64); }
    ...
    if has_short { return Ok(IrType::I16); }
    if has_char { return Ok(IrType::I8); }
    if has_double { return Ok(IrType::F64); }
    if has_float { return Ok(IrType::F32); }
...
fn lower_function_call(
    ...
    match ctx.builder.build_call(ctx.function, callee_val, arg_vals, ret_ty.clone()) {
...
fn lower_array_subscript_lvalue(
    ...
    let elem_ty = infer_array_element_type(ctx, array);
    Ok(ctx.builder.build_gep(ctx.function, base, vec![idx], elem_ty, true))
}
...
fn lower_member_access_lvalue(
    ...
    let zero = ctx.builder.build_const_int(ctx.function, IrType::I32, 0);
    let idx_val = ctx.builder.build_const_int(ctx.function, IrType::I32, field_idx as i64);

    Ok(ctx.builder.build_gep(ctx.function, base_addr, vec![zero, idx_val], base_struct_ty, true))
}
...
fn infer_base_struct_type(
    ...
    IrType::Struct { fields: vec![IrType::I32], packed: false }
}
...
fn lower_conditional(
    ...
    ctx.builder.build_cond_branch(ctx.function, cond_bool, then_bb, else_bb);
    ...
    ctx.builder.add_phi_incoming(ctx.function, phi, then_val, then_exit);
    ctx.builder.add_phi_incoming(ctx.function, phi, else_val, else_exit);
...
fn lower_compound_assign(
    ...
    let op = if is_ir_float(&elem_ty) { float_op } else { int_op };
    let new_val = ctx.builder.build_binop(ctx.function, op, old_val, rhs_coerced, elem_ty);
    ctx.builder.build_store(ctx.function, new_val, addr);
    Ok(new_val)
}
...
fn lower_compound_assign_signaware(
    ...
    let op = if is_ir_float(&elem_ty) { float_op } else { signed_op };
    let new_val = ctx.builder.build_binop(ctx.function, op, old_val, rhs_coerced, elem_ty);
    ctx.builder.build_store(ctx.function, new_val, addr);
    Ok(new_val)
}
...
fn lower_sizeof(
    ...
    Ok(ctx.builder.build_const_int(ctx.function, result_ty, size_bytes as i64))
}
...
fn lower_alignof(
    ...
    Ok(ctx.builder.build_const_int(ctx.function, result_ty, alignment as i64))
}
...
fn lower_compound_literal(
    ...
    let alloca = ctx.builder.build_alloca(ctx.function, ir_ty.clone(), Some("compound_literal"));
    ...
    match initializer {
        ...
        Initializer::List { items, .. } => {
            for (i, item) in items.iter().enumerate() {
                ...
                let zero = ctx.builder.build_const_int(ctx.function, IrType::I32, 0);
                let idx = ctx.builder.build_const_int(ctx.function, IrType::I32, i as i64);
                let elem_ptr = ctx.builder.build_gep(
                    ctx.function, alloca, vec![zero, idx], ir_ty.clone(), true,
                );
...
fn lower_label_address(
    ...
    Ok(ctx.builder.build_global_ref(ctx.function, &block_addr_name, IrType::Ptr))
}
